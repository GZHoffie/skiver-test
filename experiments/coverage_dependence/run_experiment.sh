...
for id in ${read_identity[@]}; do
  ...
  for cov in ${subsample_coverage[@]}; do
    ...
    for exp_num in $(seq 1 $num_experiments); do
      ...
      ${kvmer_dir} analyze ./temp.fastq --bidirectional --hazard-ratio ${output_dir}/${output_prefix}_hazard_ratio.csv -o ${output_dir}/${output_prefix}_verbose_output.csv > ${output_dir}/${output_prefix}.csv
      ${kvmer_dir} analyze ./temp.fastq --bidirectional -r ${reference_genome} -l 1 --use-all --hazard-ratio ${output_dir}/${output_prefix}_ref_hazard_ratio.csv -o ${output_dir}/${output_prefix}_ref_verbose_output.csv > ${output_dir}/${output_prefix}_ref.csv
